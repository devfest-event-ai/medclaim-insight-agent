...
"anomaly_reason":str_or_null}"""
    ...
    # Simple FMR
    checks = {
        "premium":   abs(extracted.get("premium_eur",0) - cleaned["premium"]) < cleaned["premium"]*0.05+0.01,
        "cost":      abs(extracted.get("cost_claims_eur",0) - cleaned["cost_claims_year"]) < cleaned["cost_claims_year"]*0.05+0.01,
        "age":       str(extracted.get("patient_age","")) == str(cleaned["age"]),
        "gender":    str(extracted.get("patient_gender","")).upper()[:1] == cleaned["gender"],
        "services":  str(extracted.get("n_medical_services","")) == str(cleaned["n_medical_services"]),
        "lr":        abs(extracted.get("loss_ratio",0) - cleaned["loss_ratio"]) < 0.1,
    }
